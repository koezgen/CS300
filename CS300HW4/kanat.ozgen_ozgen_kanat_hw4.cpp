...
void binarySearch(vector<WordItem*> vec, string word)
{
    int left = 0;
    ...
    while (left <= right)
    {
        if (vec[mid]->word == word)
        {
            break;
        }

        else if (vec[mid]->word < word)
        {
            left = mid + 1;
        }
...
void QueryDocuments(const vector<WordItem*>& myVec, string input_line)
{
    vector<string> parsed_input = return_parsed_input(input_line);
    for (int i = 0; i < parsed_input.size(); i++)
    {
         binarySearch(myVec, parsed_input[i]);
    }
}

void quickSort(vector<WordItem*> &vec, int left, int right)
{
    int i = left;
    int j = right;
    ...
    while (i <= j)
    {
        while (vec[i]->word < vec[pivot]->word)
        {
            i++;
        }

        while (vec[j]->word > vec[pivot]->word)
        {
            j--;
        }
...
void mergeSort(vector<WordItem*> &vec, int left, int mid, int right)
{
    int i, j, k;
    int n1 = mid - left + 1;
    int n2 = right - mid;
...
void mergeSort(vector<WordItem*> &vec, int left, int right)
{
    if (left < right) {
...
void heapify(vector<WordItem*> &vec, int n, int i)
{
    ...
    if ((left < n) && (vec[left]->word > vec[largest]->word))
    {
        largest = left;
    }

    if ((right < n) && (vec[right]->word > vec[largest]->word))
    {
        largest = right;
    }
...
void heapSort(vector<WordItem*> &vec) {
    int n = vec.size();
...
void insertionSort(vector<WordItem*> &vec)
{
    int i, j;
    WordItem* key;
...
int main()
{
    ...
    // Vector of vectors is here.
    vector<WordItem*> myVec[4];
    ...
    start = std::chrono::high_resolution_clock::now();
    quickSort(myVec[0], 0, myVec[0].size() - 1);
    auto QSTimeM = std::chrono::duration_cast<std::chrono::nanoseconds>
            (std::chrono::high_resolution_clock::now() - start);

    start = std::chrono::high_resolution_clock::now();
    quickSort(myVec[0], 0, myVec[0].size() - 1);
    auto QSTimeR = std::chrono::duration_cast<std::chrono::nanoseconds>
            (std::chrono::high_resolution_clock::now() - start);

    start = std::chrono::high_resolution_clock::now();
    quickSort(myVec[0], 0, myVec[0].size() - 1);
    auto QSTimeF = std::chrono::duration_cast<std::chrono::nanoseconds>
            (std::chrono::high_resolution_clock::now() - start);

    start = std::chrono::high_resolution_clock::now();
    mergeSort(myVec[1], 0, myVec[1].size() - 1);
    ...
    for (int i = 0; i < k; i++)
    {
        QueryDocuments(myVec[0], input_line);
    }
    auto BSearchTime = std::chrono::duration_cast<std::chrono::nanoseconds>
            (std::chrono::high_resolution_clock::now() - start);

    cout << "\nBinary Search Tree Time: " << BSTTime.count() / k;
    cout << "\nHash Table Time: " << HTTime.count() / k;
    cout << "\nBinary Search Time: " << BSearchTime.count() / k << "\n";

    cout << "\nQuick Sort(median) Time: " << QSTimeM.count();
    cout << "\nQuick Sort(random) Time: " << QSTimeR.count();
    cout << "\nQuick Sort(first) Time: " << QSTimeF.count();
    cout << "\nMerge Sort Time: " << MSTime.count();
    cout << "\nHeap Sort Time: " << HSTime.count();
    cout << "\nInsertion Sort Time: " << ISTime.count() << "\n";

    cout << "\nSpeed Up BST/HST: " << ((double) BSTTime.count()) / ((double) HTTime.count());
    cout << "\nSpeed Up Merge Sort/Quick Sort(Median): " << ((double) MSTime.count()) / ((double) QSTimeM.count());
    cout << "\nSpeed Up Heap Sort/Quick Sort(Median): " << ((double) HSTime.count()) / ((double) QSTimeM.count());
    cout << "\nSpeed Up Insertion Sort/Quick Sort(Median): " << ((double) ISTime.count()) / ((double) QSTimeM.count()) << "\n";

    cout << "\nSpeed Up Binary Search Tree Time / Binary Search Tree Time: " << ((double) BSTTime.count()) / ((double) BSearchTime.count());
    cout << "\nSpeed Up Binary Search / Hash Table Time: " << ((double) BSearchTime.count()) / ((double) HTTime.count()) << "\n";
